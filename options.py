...
class Options(object):
    ...
    def initialize(self):
        ...
        # default option
        parser.add_argument('--model', type=str, default='ULMEGAN', help='load ULMEGAN model')
        ...
        parser.add_argument('--resize_or_crop', type=str, default='none',
                            help='Preprocessing image, [resize_and_crop|crop|none]')
        ...
        parser.add_argument('--visdom_port', type=int, default=8088, help='visdom port.')

        # main option
        parser.add_argument('--mode', type=str, default='train', help='Mode of code. [train|test]')
        parser.add_argument('--data_root', type=str, help='paths to data set.')
        ...
        # test option
        parser.add_argument('--pre_treatment', action='store_true', help='it is necessary, one case you canot use it,when you want generate 1v1 image.')
        parser.add_argument('--re_code', action='store_true', help='remove the influence of inaccurate detection.')
        ...
        parser.add_argument('--linear_fitting', action='store_true',
                            help='replace with a straight line from the start frame to the apex frame.')
        parser.add_argument('--apex_frame', action='store_true',
                            help='replace with a straight line from the source image to the apex frame.')
        parser.add_argument('--factor', type=float, default=1.0, help='expression magnification factor.')
        parser.add_argument('--save_video', action='store_true', help='save images as video.')
        parser.add_argument('--interpolate_len', type=int, default=1, help='interpolate length for test gif.')
        parser.add_argument('--top_frame', type=int, default=0,
                            help='location of the Top frame,you can set the value when you know this.')
        ...
        parser.add_argument('--sample_img_freq', type=int, default=500, help='draw image every sample_img_freq step.')
    ...
    def parse(self):
        ...
        if opt.mode == "test":
            ...
            if opt.linear_fitting:
                out_mode ='linear_fitting'
            elif opt.apex_frame:
                out_mode = 'apex_frame'
            else:
                out_mode = '1V1'
            if opt.re_code:
                ...
                recode_name = 'no-re_code'
            opt.results = os.path.join(opt.results, "%s_%s_%s_%s_%s" % (dataset_name, opt.model, opt.load_epoch,recode_name,out_mode ))
